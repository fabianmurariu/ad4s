...
trait TapeEvaluatorMagnet[A, B] {
  ...
  def eval(x: A)
          (implicit BC: BackpropContext, B: Backprop[B]): (d[B], Grad => Grads)
}
...
object TapeEvaluatorMagnet {
  ...
  object Implicits {
    ...
    implicit def liftFn2IntoMagnet[T](f: (d[T], d[T]) => d[T]) = new TapeEvaluatorMagnet[(T, T), T] {
      override type Grads = (T, T)

      override def eval(x: (T, T))
                       (implicit BC: BackpropContext, B: Backprop[T]): (d[T], Grad => (T, T)) = {
