...
object TapeEvaluatorMagnet {
  ...
  implicit def liftFn1IntoMagnet[A: Backprop, B](f: d[A] => d[B]) = new TapeEvaluatorMagnet[A, B] {
  ...
  implicit def liftFn2IntoMagnet[A: Backprop, B: Backprop, C](f: (d[A], d[B]) => d[C]) = new TapeEvaluatorMagnet[(A, B), C] {
